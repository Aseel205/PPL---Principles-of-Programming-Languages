...
import { isArray, isString, isNumericString, isIdentifier, Identifier } from "../shared/type-predicates";
...
export type CExp =  AtomicExp | CompoundExp;  /// this is called also literal expression
...
export const makeClassExp=(   fields:VarDecl[], methods:Binding[]):ClassExp =>
    ({tag: "ClassExp", fields:fields, methods:methods}); //aseel
...
export const isClassExp = (x: any): x is ClassExp => x.tag === "ClassExp"; //aseel
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x)||isClassExp(x); //aseel
...
const parseClassExp = (params: Sexp[]): Result<ClassExp> =>
    isEmpty(params) ? makeFailure("Empty class definition") :
    params.length !== 2 ? makeFailure("Class definition should have exactly two elements") :
    !isArray(params[0]) || !allT(isIdentifier, params[0]) ? makeFailure(`Class fields must be a list of identifiers": ${format(params[0])}`) :
    isArray(params[1]) && allT(isNonEmptyList, params[1]) && allT(isGoodBinding, params[1]) ?
        mapv(parseBindings(params[1]), (bindings: Binding[]) =>
            makeClassExp(map(makeVarDecl, params[0] as Identifier[]), bindings)) :
    makeFailure(`Class methods must be a list of bindings: ${format(params[1])}`);
    ...
    const isGoodBinding = (binding: Sexp): binding is [string, Sexp] =>
        isNonEmptyList(binding) && binding.length === 2 && isIdentifier(binding[0]);
        
        const parseBindings = (bindings: Sexp[]): Result<Binding[]> =>
            mapResult(binding => parseBinding(binding[0], binding[1]), bindings);
        
        const parseBinding = (v: Sexp, val: Sexp): Result<Binding> =>
            !isIdentifier(v) ? makeFailure(`Invalid binding variable: ${v}`) :
            mapv(parseL3CExp(val), (value: CExp) => makeBinding(v, value));
...
const unparseClassExp = (ce: ClassExp): string =>
    `(class (${map((field: VarDecl) => field.var, ce.fields).join(" ")}) ${unparseBindings(ce.methods)})`

const unparseBindings = (bindings: Binding[]): string =>                     
    `(${map((b: Binding) => 
        isProcExp(b.val) 
        ? `(${b.var.var} (lambda (${map((arg: VarDecl) => arg.var, b.val.args).join(" ")}) ${unparseLExps(b.val.body)}))` 
        : `(${b.var.var} ${unparseL3(b.val)})`, bindings).join(" ")})`
...
export const unparseL3 = (exp: Program | Exp): string =>
    ...
    isClassExp(exp) ? unparseClassExp(exp) : // New case for ClassEx    
    exp;
