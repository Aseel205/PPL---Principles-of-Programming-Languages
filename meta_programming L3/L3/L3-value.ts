...
const extractMethodNames = (methods: Binding[]): string[] => 
    methods.map(method => (method.var).var);
...
export const makeClassInstance = (cls: Class, fieldValues: Value[]):  ClassInstance =>
    ({ tag: "ClassInstance", class: cls, fieldValues: fieldValues,env : makeEmptyEnv()  });

export const isClassInstance = (x: any): x is ClassInstance => x.tag === "ClassInstance"

export const makeClassInstanceEnv = (cls: Class, fieldValues: Value[],env : Env ): ClassInstance => {
    return { 
        tag: "ClassInstance", 
        class: cls, 
...
export type SExpValue = number | boolean | string | PrimOp | Closure | SymbolSExp | EmptySExp | CompoundSExp|Class|ClassInstance;
