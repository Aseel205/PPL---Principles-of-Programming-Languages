...
import { isClosure, makeClosureEnv, Closure, Value, makeClass, Class, isClass, ClassInstance, makeClassInstanceEnv, isClassInstance, SymbolSExp } from "./L3-value";
...
const createClassInstanceEnv = (exp: Class, args: Value[], env: Env): Result<ClassInstance> =>
    args.length === exp.fields.length ? makeOk(makeClassInstanceEnv(exp, args, env))
        : makeFailure('invalid filed number');


const activateClassFunctionEnv = (exp: ClassInstance, args: Value[]): Result<Value> =>
...
const applyProcedure = (proc: Value, args: Value[], env: Env): Result<Value> => {
    ...
    return isPrimOp(proc) ? applyPrimitive(proc, args) :
        isClosure(proc) ? applyClosure(proc, args) :
            isClass(proc) ? createClassInstanceEnv(proc, args, env) :
                isClassInstance(proc) ? activateClassFunctionEnv(proc, args) :
