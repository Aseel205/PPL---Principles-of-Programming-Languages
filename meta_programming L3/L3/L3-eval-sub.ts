...
import { isClosure, makeClosure, Closure, Value, Class, makeClass, ClassInstance, isClassInstance, makeClassInstance, isSymbolSExp, isClass, SymbolSExp } from "./L3-value";
...
const createClassInstance = (exp: Class, args: Value[]): Result<ClassInstance> =>     // (pair 3 4)
    args.length === exp.fields.length ? makeOk(makeClassInstance(exp, args))
        : makeFailure('invalid filed number');




const activateClassFunction = (exp: ClassInstance, args: Value[], env: Env): Result<Value> => {
...
export const addValuesToEnv = (cls: ClassInstance, env: Env): Env => 
...
const L3applyProcedure = (proc: Value, args: Value[], env: Env): Result<Value> =>
    isPrimOp(proc) ? applyPrimitive(proc, args) :
        isClosure(proc) ? applyClosure(proc, args, env) :
            isClass(proc) ? createClassInstance(proc, args) :
                isClassInstance(proc) ? activateClassFunction(proc, args, env) :
