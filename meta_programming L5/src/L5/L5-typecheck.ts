// L5-typecheck
//  return the type  of the whole porgram     
// we start from the function typeof
// ========================================================
...
import {
    makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
    parseTE, unparseTExp, makeUnionTExp,
    BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isSubType,

    makePredicateProcTExp,
    isBoolTExp,
    islambdaTExp,
    isPredicateTExp,
    isPredicateProcTExp,
...
import { Result, makeFailure, bind, makeOk, zipWithResult, either, isOk, Ok } from '../shared/result';
...
export const checkCompatibleType = (te1: TExp, te2: TExp, exp: Exp): Result<true> => {
    // log(" we are in checkCompatibleType")

    return isSubType(te1, te2) ? makeOk(true) :
...
const isTypePredApp = (e: Exp, tenv: TEnv): Result<boolean> => {
    ...
    return !isAppExp(e) ? makeOk(false) :
        !isVarRef(e.rator) ? makeOk(false) :
            !isOk((applyTEnv(tenv, e.rator.var))) ? makeOk(false) :
                !isPredicateProcTExp(((applyTEnv(tenv, e.rator.var)) as Ok<TExp>).value) ? makeOk(false) :
                    makeOk(true)
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> => {
    ...
    return (isTypePredApp(ifExp.test, tenv) as Ok<boolean>).value ? handleTypePredIf(ifExp, tenv) : typeofIfNormal(ifExp, tenv)
}

const handleTypePredIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> => {

    const varx = (ifExp.test as AppExp).rands[0]
    const varx_var = ((ifExp.test as AppExp).rands[0] as VarRef).var


    makeExtendTEnv([varx_var], [makeNumTExp()], tenv)
    const testTE = typeofExp(ifExp.test, tenv);
    const thenTE = typeofExp(ifExp.then, makeExtendTEnv([varx_var], [makeNumTExp()], tenv));
    const altTE = typeofExp(ifExp.alt, tenv);
    const constraint1 = bind(testTE, testTE => checkCompatibleType(testTE, makeBoolTExp(), ifExp));
    // L51
    const constraint2 = bind(thenTE, (thenTE: TExp) =>
...
export const typeofProc = (proc: ProcExp, tenv: TEnv): Result<TExp> => {
    ...
    const makeTExp = (returnTE: TExp) => isPredicateTExp(returnTE)
        ? makeOk(makePredicateProcTExp(argsTEs, returnTE))
        : bind(constraint1, _ => makeOk(makeProcTExp(argsTEs, returnTE)));

    return makeTExp(proc.returnTE);
};
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> => {
    return bind(typeofExp(app.rator, tenv), (ratorTE: TExp) => {
        ...
        if (isPredicateTExp(ratorTE)) {
            return bind(constraints, _ => makeOk(makeBoolTExp()))
        }
        else {
...
export const typeofProgram = (exp: Program, tenv: TEnv): Result<TExp> => {
    ...
    return typeofSequence(exp.exps, tenv);
}
// Purpose: Compute the type of a sequence of expressions
// Check all the exps in a sequence - return type of last.
...
export const typeofSequence = (seq: List<Exp>, tenv: TEnv): Result<TExp> => {
    ...
    return isNonEmptyList<Exp>(seq) ? typeofSequenceFirst(first(seq), rest(seq), tenv) :
