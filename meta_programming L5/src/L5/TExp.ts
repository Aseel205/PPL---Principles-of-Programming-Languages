...
*/
// here  the proc expression must be a disjoint Union 

import { all, append, chain, concat, equals, map, sort, uniq, zip, filter, includes, union, is } from "ramda";
...
import { Result, bind, makeOk, makeFailure, mapResult, mapv, either, Ok } from "../shared/result";
import { parse as p } from "../shared/parser";
import { format } from "../shared/format";
import { isObject } from "util";

export type TExp = AtomicTExp | CompoundTExp | TVar | PredicateTExp
...
export type CompoundTExp = lambdaTExp | TupleTExp | UnionTExp | InterTExp;
export const isCompoundTExp = (x: any): x is CompoundTExp => isProcTExp(x) || isTupleTExp(x) || isUnionTExp(x) || isInterTExp(x);


export type lambdaTExp = PredicateProcTExp | ProcTExp
export const islambdaTExp = (x: any): x is lambdaTExp => isProcTExp(x) || isPredicateProcTExp(x)
...
export type PredicateTExp = {
    tag: "PredicateTExp";
    predType: TExp; // The type being checked by the predicate
};

export const makePredicateTExp = (predType: TExp): PredicateTExp => ({
    tag: "PredicateTExp",
    predType
});

export const isPredicateTExp = (texp: TExp): texp is PredicateTExp => texp.tag === "PredicateTExp";
...
export type PredicateProcTExp = { tag: "PredicateProcTExp"; paramTEs: TExp[]; returnTE: TExp; }; //  this is the parmaters and the returnTE

export const makePredicateProcTExp = (paramTEs: TExp[], returnTE: TExp): PredicateProcTExp =>
    ({ tag: "PredicateProcTExp", paramTEs: paramTEs, returnTE: returnTE });

export const isPredicateProcTExp = (x: any): x is PredicateProcTExp => x.tag === "PredicateProcTExp";
...
export const makeUnionTExp = (tes: TExp[]): TExp => {
    ...
    return normalizeUnion(({ tag: "UnionTExp", components: flattenSortUnion(tes) }));
}
...
export const makeInterTExp = (tes: TExp[]): TExp => {
    ...
    return (dnf({ tag: "InterTExp", components: removeAny(tes) }));

}
export const isInterTExp = (x: any): x is InterTExp => x.tag === "InterTExp";


const equalss = (te1: TExp, te2: TExp): boolean => {
...
const flattenSortInter = (tes: TExp[]): TExp[] =>
    (flattenInter((sort(subTypeComparator, tes))))
...
const removeAny = (tes: TExp[]): TExp[] =>
    tes.filter(te => !isAnyTExp(te));
...
export const makeDnf = (disj: UnionTExp[], factors: TExp[]): TExp =>
    isEmpty(disj) && isEmpty(factors) ? makeAnyTExp() :
        isEmpty(disj) ? ({ tag: "InterTExp", components: (flattenSortInter(factors)) }) :
...
export const isSubType = (te1: TExp, te2: TExp): boolean => {
    ...
    return (isAnyTExp(te2) || isNeverTExp(te1)) ? true :
        isPredicateTExp(te1) && isBoolTExp(te2) ? true :
            (isProcTExp(te1) && isProcTExp(te2)) ? checkProcTExps(te1, te2) :
                (isAtomicTExp(te1) && isAtomicTExp(te2)) ? equalss(te1, te2) :
                    isTVar(te1) ? equals(te1, te2) :  // this case I dont understand

                        isInterTExp(te1) && !te1.components.every((exp) => exp.tag === (te1 as InterTExp).components[0].tag) ? true :
                            isInterTExp(te1) ? isSubType(te1.components[0], te2) :
                                isUnionTExp(te1) ? te1.components.every((comp) => isSubType(comp, te2)) :
                                    isInterTExp(te2) ? te2.components.every((comp) => isSubType(te1, comp)) :
                                        isUnionTExp(te2) ? te2.components.some((comp) => isSubType(te1, comp)) :
                                            false;
...
export const containsType = (tes: TExp[], te: TExp): boolean =>
    isEmpty(tes) ? false :
        (isSubType(te, tes[0]) && (!isInterTExp(tes[0]))) ? true :
                ...
                (texp === "string") ? makeOk(makeStrTExp()) :
                    (texp === "any") ? makeOk(makeAnyTExp()) :
                        (texp === "never") ? makeOk(makeNeverTExp()) :
                            (isString(texp) && texp.includes("is?")) ? parsePredicateTExp(texp) : // Handle type predicate
                                isString(texp) ? makeOk(makeTVar(texp)) :
                                    isArray(texp) ? parseCompoundTExp(texp) :
                                        makeFailure(`Unexpected TExp - ${format(texp)}`);

}
// Helper function to parse PredicateTExp
const parsePredicateTExp = (texp: string): Result<TExp> => {

    const predTypeStr = texp.replace(/^is\?\s*/, ''); // Remove "is?" and any following whitespace
    return bind(parseTExp(predTypeStr), (predType: TExp) =>
        makeOk(makePredicateTExp(predType))
    );
};
...
const parseProcTExp = (texps: Sexp[]): Result<lambdaTExp> => {
                ...
                (texps.slice(pos + 1).indexOf('->') > -1) ? makeFailure(`Only one -> allowed in a procexp - ${format(texps)}`) :
                    bind(parseTupleTExp(texps.slice(0, pos)), (args: TExp[]) =>
                        mapv(parseTExp(texps[pos + 1]), (returnTE: TExp) =>
                            isPredicateTExp(returnTE)
                                ? makePredicateProcTExp(args, returnTE)
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>    /// this funciton for unparsing ..
                                ...
                                isEmptyTVar(x) ? makeOk(x.var) :
                                    isTVar(x) ? up(tvarContents(x)) :
                                        isUnionTExp(x) ? mapv(mapResult(unparseTExp, x.components), (componentTEs: string[]) =>
                                            ...
                                            islambdaTExp(x) ? bind(unparseTuple(x.paramTEs), (paramTEs: string[]) =>
                                                ...
                                                isEmptyTupleTExp(x) ? makeOk("Empty") :
                                                    isNonEmptyTupleTExp(x) ? unparseTuple(x.TEs) :
                                                        x === undefined ? makeFailure("Undefined TVar") :
                                                            isInterTExp(x) ? makeOk((up(x.components[0]) as Ok<string>).value) :  // pass
                                                                isPredicateTExp(x) ? makeOk("is?" + x.predType + "") :// here you must change it 
                                                                    x;
