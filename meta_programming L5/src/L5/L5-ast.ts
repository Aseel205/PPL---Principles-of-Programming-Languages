...
// you have to do predicaete expression
...
export type PredExp = { tag: "PredExp"; var: VarDecl; pred: CExp; }
export const makePredExp = (v: VarDecl, pred: CExp): PredExp => ({ tag: "PredExp", var: v, pred: pred });
export const isPredExp = (x: any): x is PredExp => x.tag === "PredExp";
...
export const parseL5Program = (sexp: Sexp): Result<Program> => {
    ...
    return isToken(sexp) ? makeFailure(`Program cannot be a single token: ${format(sexp)}`) :
...
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {
    ...
    if (isArray(vars)) {
        const args = mapResult(parseVarDecl, vars); // Parse variable declarations
        const body = mapResult(parseL5CExp, rest[0] === ":" ? rest.slice(2) : rest); // Parse the body

        const returnTE = rest[0] === ":"
            ? (typeof rest[1] === 'string' && rest[1].startsWith("is?"))
                ? parseTExp(rest[1] + rest[2]) // Handle type predicate function and pass the whole string
                : parseTExp(rest[1]) // Handle regular function
            : makeOk(makeFreshTVar()); // Default return type
